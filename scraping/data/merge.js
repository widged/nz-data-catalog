...
let inStream = vinyl.src('./*/links.tsv', { buffer: false })
							.pipe((function() {
								let pending = 0;
								return through(
									function(file, cb) {
...
mergeStream.pipe(through(
	...
	function() {
		...
		const filename = __dirname + '/merged.nedb.json';
		if (fs.existsSync(filename)) { fs.unlinkSync(filename); }
		const filename_geo = __dirname + '/merged_geo.nedb.json';
